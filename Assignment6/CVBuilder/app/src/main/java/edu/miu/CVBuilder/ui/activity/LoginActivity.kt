...
import CVBuilder.R
import CVBuilder.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    fun onLogin(view: View) {
        ...
        if(user.isEmpty()){
            Toast.makeText(applicationContext,"Username can't be blank", Toast.LENGTH_LONG).show()
            return
        }
        if(pass.isEmpty()){
            Toast.makeText(applicationContext,"Password can't be blank.", Toast.LENGTH_LONG).show()
            return
        }
